...
-- Fix blocks table - add name column if not exists
ALTER TABLE blocks ADD COLUMN IF NOT EXISTS name TEXT;

-- Fix assignments table structure
ALTER TABLE assignments ADD COLUMN IF NOT EXISTS block_id UUID REFERENCES blocks(id) ON DELETE CASCADE;
...
-- Create index on block_id if not exists
CREATE INDEX IF NOT EXISTS idx_assignments_block_id ON assignments(block_id);
...
CREATE POLICY "Admins and dirigentes can insert blocks" ON blocks
  FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'dirigente'))
  );

DROP POLICY IF EXISTS "Admins can delete blocks" ON blocks;
CREATE POLICY "Admins and dirigentes can delete blocks" ON blocks
  FOR DELETE USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'dirigente'))
  );
